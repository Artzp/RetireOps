...
- Use the hosted app when it is available.
...
- People can use, study, modify, and self-host it for free.
- If someone runs a modified hosted version, they must also share those changes.
- It fits the goal of being open source for personal use while still allowing a public web version.

If you later want to offer commercial licensing for proprietary deployments, you can still do that as the copyright holder.
...
Individuals can self-host RetireOps for free. If you deploy it publicly for other users, review:
...
A hosted version of this codebase can be offered under AGPL-3.0. The main condition: if you modify the software and let users interact with it over a network, those users must be able to obtain the source for the version you are running.
